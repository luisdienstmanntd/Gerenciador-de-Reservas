...
| `js/core/database.js` | **v2.1** | 2026-07-14 — `_paraColunasReserva()` não inclui mais `mesa_identificador` (bug #63). 2026-07-10 — métodos de `config_sistema` (bug #56), update-not-duplicate de hóspede na edição (bug #55), `codigoReserva` no formato achatado (bug #54), `bloqueioOrigemId` + `buscarBloqueiosAutomaticos()` (bug #52) |
| `js/core/init.js` | **v2.3** | 2026-07-14 — mensagens de `modalConfirmar` revisadas: "sem disponibilidade" em linhas separadas, exclusão de reserva mostra o nome e reforça que é definitivo (bug #67). Branch "Alterar horário" do `btnSalvar` pergunta antes de criar linha nova quando o destino está sem linha base livre (bug #65). 2026-07-10 — chama `iniciarEscutaConfigSistema()` no boot (bug #56) |
...
| `js/features/reservas/modal.js` | **v3.17** | 2026-07-14 — mensagem de "sem disponibilidade" em linhas separadas, mesmo padrão de `init.js` (bug #67). `btnConfirmarData` trata o erro `semDisponibilidade` de `alterarData()` (bug #66). `recarregarReservas()` após `alterarData()` (bug #64). 2026-07-13 — menus de resumo e o seletor de ação (`abrirNova`) redesenhados com ícones SVG lineares + classes `.resumo-btn` (bugs #60, #61) |
| `js/features/reservas/service.js` | **v4.5** | 2026-07-16 — `aplicarBloqueiosSemanais()`: bloqueios antecipados nos dias de movimento (item #69). 2026-07-14 — `alterarData()` ganha a mesma checagem de capacidade/aviso de `salvarApenasHorario()` (bug #66). `salvarApenasHorario()` busca posição livre a partir de 0 no bloco de destino, não mais a partir da posição antiga; lança `semDisponibilidade` quando as linhas base do destino estão todas ocupadas (bug #65). 2026-07-10 — bloqueio automático de reserva grande (bugs #52, aplica em várias linhas + reavaliação após desbloqueio manual), respeitando o switch de Configurações (bug #56) |
...
| `js/ui/controls.js` | **v9.2** | 2026-07-16 — lista de regras de "Bloqueios Antecipados" também fica oculta até o switch ligar, não só o botão de adicionar (item #71). Editor com trava por switch (item #69, ajuste #70). 2026-07-10 — switch de travar/destravar em Configurações, switch de bloqueio automático, `salvarConfiguracoes()` grava em `config_sistema` (Supabase) em vez de `localStorage` (bugs #52, #56) |
...
| `css/*.css` (8 arquivos) | — | 2026-07-12 — `style.css` único (2151 linhas) dividido em 8 arquivos temáticos (base/layout/reservas/modais/dashboard/mesas-timers/responsive/home), cascata preservada pela ordem dos `<link>`. Equivalência verificada: 238 regras idênticas, na mesma ordem. 2026-07-13 — `css/modais.css` ganha as classes `.resumo-btn`/`.resumo-lista` reaproveitadas pelo modal de mesa e pelo modal "GERENCIAR" (bug #61); `.btn-main`/`.save-btn`/`.cancel-btn`/`.delete-btn`/`.btn-time` (SALVAR/CANCELAR/EXCLUIR/DESBLOQUEAR do formulário de reserva + setas ▲▼ do relógio) trocam de preenchimento sólido pra contorno + acento colorido, mesmo padrão visual (bug #62) |
...
| 60 | Solicitação: menu de ações da reserva (resumo) tinha botões sólidos, texto em caixa alta e sem ícone — visual "carregado", pouco profissional, alvos de toque apertados pra tablet | `js/features/reservas/modal.js`, `css/modais.css` | Redesenho dos 4 menus de resumo (normal, cancelada, mesa em atendimento, mesa finalizada): ícones SVG lineares (stroke, mesmo padrão de `log.js`) + classes `.resumo-btn`/`.resumo-linha-dupla` no lugar de estilo inline. Visual "cartão neutro + faixa de acento colorido por ação" em vez de botão sólido pintado inteiro; altura mínima 52px (alvo de toque); Horário/Data e Cancelar/Excluir agrupados em duas colunas |
| 61 | Solicitação: estender o redesenho do bug #60 pros outros modais do sistema — seletor de ação da linha disponível (NOVA RESERVA/BLOQUEAR/ALTERAR HORÁRIO), modal de mesa (`modalMesa`) e o modal "GERENCIAR" de horário (`menuHorario`) — pra deixar o sistema inteiro mais profissional | `js/features/reservas/modal.js` (`abrirNova()`), `index.html` (`modalMesa`, `menuHorario`), `css/modais.css` | Mesmo padrão `.resumo-lista`/`.resumo-btn` + ícones SVG lineares aplicado aos 3 modais restantes. 2 ícones novos (`nova`, `bloquear`) adicionados ao objeto `ICONES` de `modal.js`; os botões estáticos de `modalMesa`/`menuHorario` (que já usavam IDs fixos e `classList.add/remove('hidden')`, sem depender das classes antigas `btn-main`/`save-btn`/`delete-btn`/`cancel-btn` para lógica) tiveram só a classe trocada, sem tocar em `mesas/modal.js` |
| 62 | Solicitação: os últimos botões "carregados" do sistema — SALVAR/CANCELAR/EXCLUIR/DESBLOQUEAR do formulário de reserva (`btnSalvar`/`.cancel-btn`/`btnExcluir`) e as setas ▲▼ de ajuste do relógio (ALTERAR HORÁRIO) — ainda usavam preenchimento sólido (verde/cinza/vermelho/vinho), destoando do padrão contorno+ícone já aplicado nos bugs #60/#61 | `css/modais.css` | `.btn-main`/`.save-btn`/`.cancel-btn`/`.delete-btn`/`.btn-time` redesenhados: fundo `var(--card-bg)`, borda 1px na cor de acento (verde/vermelho conforme a ação, neutra pro cancelar), texto na cor do acento em vez de branco sobre fundo sólido — mesmo espírito visual do `.resumo-btn`, sem duplicar a classe (esses botões continuam num grid 2 colunas, não numa lista vertical, então mantiveram sua estrutura própria). Nenhuma classe/id mudou — só a aparência; `mesas/modal.js` e `reservas/modal.js` não precisaram de alteração |
| 63 | Reportado em teste manual: destinar uma mesa a uma reserva e depois editar qualquer dado da reserva fazia a mesa "sumir" — precisava selecionar de novo. Causa raiz: `_paraColunasReserva()` (helper que traduz o objeto achatado da UI pras colunas do Postgres, usado tanto por `criarReserva` quanto por `atualizarReserva`) sempre recalculava `mesa_identificador` a partir de `dados.mesa`. O formulário de edição de reserva nunca tem esse campo — mesa é atribuída só pelo modal "SELECIONAR MESA", um fluxo totalmente separado (`atribuirMesa()`) — então `dados.mesa` vinha sempre `undefined`, e todo `UPDATE` de edição mandava `mesa_identificador: null`, apagando a mesa mesmo sem o usuário tocar nela | `js/core/database.js` | `mesa_identificador` removido do objeto que `_paraColunasReserva()` monta — como o Supabase só sobrescreve as colunas presentes no `.update({...})`, um `UPDATE` de edição de reserva agora nem menciona essa coluna, deixando-a como estava. A mesa só muda através de `atribuirMesa()`/`cancelarMesa()` (service.js), que fazem `UPDATE` isolados direto na coluna e continuam funcionando normalmente. Testado manualmente pelo usuário em produção: mesa permanece após editar a reserva |
| 64 | Reportado em teste manual: ao trocar a data de uma reserva (ex: 13/07 → 14/07) via "ALTERAR DATA", a reserva passava a aparecer corretamente no dia 14/07, mas continuava aparecendo também no dia 13/07 até o usuário trocar manualmente a data visualizada na tela (ida e volta, ou reload). Causa raiz: a escuta em tempo real de reservas (`escutarReservasPorData`) filtra por `data=eq.<data visualizada>` no Supabase Realtime. Quando `alterarData()` faz um `UPDATE` que muda justamente a coluna `data` (a mesma usada no filtro), a linha deixa de casar com o filtro do canal antigo — o Postgres Realtime avalia o filtro contra o valor NOVO da linha pra eventos de UPDATE, então o canal da data de origem nunca recebe esse evento (não existe um "DELETE sintético" quando uma linha só sai do escopo de um filtro). Mesma classe de limitação do Realtime já vista no bug #58 (mas ali a coluna que mudava não era a filtrada) | `js/features/reservas/modal.js` | Depois de `alterarData()` ter sucesso, o handler do botão CONFIRMAR chama `recarregarReservas(getDataAtual())` — força um refetch completo da data que está sendo visualizada, sem depender do Realtime notificar essa transição. Mesmo padrão já usado pro resync ao voltar pra aba (`visibilitychange` em `listener.js`). Testado manualmente pelo usuário em produção: reserva some da grade de origem imediatamente após confirmar a nova data |
| 65 | Reportado em teste manual: alterar o horário de uma reserva (ex: 22:00 → 21:30) criava uma linha nova mesmo havendo linhas base livres no horário de destino. Causa raiz: `salvarApenasHorario()` calculava a posição livre no bloco de destino usando como "posição desejada" a posição que a reserva tinha no bloco de ORIGEM (`dadosAtuais.posicao`) — um número sem nenhuma relação com o bloco novo, já que cada `originalBase` tem sua própria numeração 0-based. Se a reserva estava numa linha extra (posição 3+) no bloco de origem, `_calcularPosicaoLivre()` só troca a posição desejada por 0 quando ela está OCUPADA no destino — como a posição 3 quase nunca está ocupada lá (o bloco de destino raramente tem 4+ reservas), a função aceitava a posição 3 direto, ignorando que as posições 0-2 (as 3 linhas base) estavam livres | `js/features/reservas/service.js`, `js/core/init.js` | `salvarApenasHorario()` agora busca a posição livre sempre a partir de 0 no bloco de destino ao trocar de horário — encontra a primeira linha base livre em vez de herdar um número arbitrário do bloco antigo. Além disso, a função ganhou uma checagem de capacidade: se todas as linhas base do destino (3, ou o número de linhas extras já adicionadas) estiverem ocupadas, ela lança um erro `semDisponibilidade` em vez de criar uma linha nova sozinha. `init.js` captura esse erro e pergunta via `modalConfirmar()` — "Sem disponibilidade em `<horário>` — deseja criar uma nova linha para esta reserva?" — só cria a linha extra se o usuário confirmar (parâmetro novo `salvarApenasHorario(dados, { permitirNovaLinha: true })`) |
| 66 | Confirmado o bug #65 corrigido (aviso aparece corretamente ao alterar horário no dia atual), o usuário testou o cenário análogo em "ALTERAR DATA" — mover uma reserva pra uma data futura onde o mesmo horário já está com as linhas base cheias não mostrava aviso nenhum e criava uma linha nova sozinha. Causa raiz: a checagem de capacidade do bug #65 tinha sido implementada só em `salvarApenasHorario()` (fluxo de horário) — `alterarData()` (fluxo de data) nunca teve essa proteção, só calculava a posição livre e seguia direto pro `UPDATE`, exatamente como `salvarApenasHorario()` fazia antes da correção anterior | `js/features/reservas/service.js`, `js/features/reservas/modal.js` | `alterarData()` ganhou a mesma checagem de capacidade do bug #65: se as linhas base do horário na data de destino estiverem todas ocupadas, lança `semDisponibilidade` em vez de criar linha nova direto (aceita `alterarData(id, novaData, { permitirNovaLinha: true })` pra prosseguir depois da confirmação). O handler do botão CONFIRMAR em `modal.js` captura o erro e pergunta via `window.modalConfirmar()`, mesmo texto/padrão usado no fluxo de horário |
| 67 | Solicitação: revisar o texto de dois modais de confirmação — "sem disponibilidade" (bugs #65/#66) deveria mostrar data+horário, o motivo e a pergunta em linhas separadas em vez de uma frase corrida; o de excluir reserva deveria nomear a reserva entre aspas e deixar claro que é uma exclusão definitiva, diferente de um cancelamento (que é soft-delete, ver bug #57) | `js/core/init.js`, `js/features/reservas/modal.js` | Novo helper `_formatarDataBR()` (YYYY-MM-DD → DD/MM/AAAA) duplicado em `init.js` e `modal.js` (cada um já converte suas próprias mensagens, sem criar acoplamento novo entre os dois). Mensagem de "sem disponibilidade" vira 3 linhas via `<br>` — `<data> <horário>` / `Sem disponibilidade.` / `Deseja criar uma nova linha para esta reserva?` — nos dois fluxos (`salvarApenasHorario()` em `init.js`, `alterarData()` em `modal.js`). Mensagem de excluir vira `Excluir reserva de "<nome>"?` + `Isso apaga o registro definitivamente (diferente de cancelamentos)`, com `escapeHtml()` no nome (mesma regra 3 já aplicada em outras confirmações que interpolam dado do usuário) |
| 68 | Solicitação: revisar a cobertura de testes de todas as funções do código pra evitar quebras futuras. Descoberta no processo: a contagem "172 testes" reportada até aqui estava INFLADA — o Vitest também varria a cópia completa do projeto num worktree antigo em `.claude/worktrees/`, rodando cada teste duas vezes (86×2). A contagem real era 91 testes em 3 arquivos (validators/state/service) | `vitest.config.js`, `js/core/database.test.js` (novo), `js/core/notificacao.test.js` (novo), `js/features/reservas/log.test.js` (novo), `js/features/reservas/service.fluxos.test.js` (novo), `js/ui/render.test.js` (novo), `js/ui/controls.test.js` (novo), `js/core/state.test.js`, `js/features/reservas/service.test.js`, `js/features/reservas/service.js`, `js/features/reservas/log.js` | `vitest.config.js` ganha `test.exclude` pra `.claude/**` (corrige a duplicação). Cobertura sobe de 91 pra **168 testes em 9 arquivos**: `database.test.js` trava os mapeamentos `_paraReservaApp`/`_paraColunasReserva` (incl. regressão do bug #63 — mesa NUNCA entra no UPDATE de edição) e `_horaCurta`/`_paraConfigSistemaApp`; `service.fluxos.test.js` testa `salvarApenasHorario()`/`alterarData()` com banco mockado (`vi.mock`), travando as regras dos bugs #65/#66 (primeira linha base livre + erro `semDisponibilidade` + `permitirNovaLinha`); `render.test.js` cobre `atualizarMiniCards()`/`renderizarGrid()` (incl. regressões dos bugs #58 — cancelada nunca disputa posição — e #32 — XSS escapado); `notificacao.test.js`, `log.test.js` (`_resumir`/`_camposAlterados`/`_paraLogApp`, exportados pra teste) e `controls.test.js` (relógio ▲▼, tema, travas de config) completam. Helpers `_totalLinhasBloco`/`_linhasExtrasNecessarias` exportados (mesmo padrão de `_calcularPosicaoLivre`). **Sem teste unitário (integração/DOM pesado, cobertos por teste manual)**: bootstrap de `init.js`, listeners Realtime (`listener.js`, `escutar*` de `database.js`), classes `ReservaModal`/`MesaModal`, `navigation.js`, `home.js`/`dashboard.js` (Chart.js), `timers.js`, `filters.js`, `roomservice.js` |
| 69 | Solicitação: qui/sex/sáb são os dias de maior movimento de hóspedes no hotel — reservas de externos feitas pra datas futuras podiam consumir as vagas dos hóspedes. Pedido: algumas linhas já nascerem bloqueadas nesses dias (padrão: 1×20:00, 2×20:30, 1×21:00), com controle total em Configurações (quantas linhas, quais horários, quais dias da semana) | `supabase/migrations/20260716120000_bloqueios_semanais.sql`, `js/core/state.js`, `js/core/database.js`, `js/features/reservas/service.js`, `js/features/reservas/listener.js`, `js/ui/controls.js`, `index.html`, `css/layout.css` | Nova coluna `config_sistema.bloqueios_semanais` (jsonb `{dia 0-6: {horário: qtd}}`, sincronizada entre usuários) + flag `config_dia.bloqueios_semanais_aplicados`. `aplicarBloqueiosSemanais(data)` semeia os bloqueios (obs "BLOQUEIO ANTECIPADO — DIA DE MOVIMENTO") na primeira vez que a data é visualizada — disparada em `escutarReservas()` (troca de data) e no callback de `config_sistema` (boot, quando a config real chega; `isConfigSistemaCarregada()` em state.js impede decisão com base nos defaults). Semeadura é única por data (flag): desbloquear/excluir manualmente NUNCA é desfeito pelo sistema. Só ocupa linhas base livres (posições 0-2) — nunca atropela reserva nem abre linha extra; datas passadas nunca são semeadas. Editor em Configurações: linhas [dia da semana][horário][qtd 1-3][✕] + botão "+ ADICIONAR REGRA", serializado de volta pro jsonb no SALVAR (regra duplicada: a última vence). 8 testes novos em `service.fluxos.test.js` + `_diaSemanaDe()` em `service.test.js` (176 no total). ⚠️ Migration precisa ser aplicada no SQL Editor antes do deploy fazer efeito |
| 70 | Ajuste do item #69: texto do card revisado ("Bloqueios Antecipados — Dias de Movimento" → "Bloqueios Antecipados"; parágrafo longo → "Bloqueia horários de datas futuras") e o botão "+ ADICIONAR REGRA" deveria ficar escondido até o usuário destravar a seção, mesmo padrão de trava de capacidade/mesas (evita adicionar regra sem querer) | `index.html`, `js/ui/controls.js` | Novo switch `toggleConfigBloqueiosSemanais` no cabeçalho do card (mesmo componente `.switch`/`slider-round` de capacidade/mesas). `alternarEdicaoBloqueiosSemanais(habilitado)` alterna a classe `.hidden` do botão; `carregarConfiguracoes()` sempre abre com o switch desligado e o botão escondido, igual às travas existentes — a lista de regras já salvas continua visível/editável independente do switch, só a criação de regra nova é que exige destravar. 3 testes novos (179 no total) |
| 71 | Ajuste do item #70: não bastava esconder só o botão "+ ADICIONAR REGRA" — a LISTA de regras configuráveis também deveria ficar oculta, aparecendo só ao deslizar o switch | `js/ui/controls.js` | `alternarEdicaoBloqueiosSemanais(habilitado)` passa a alternar a classe `.hidden` também em `#listaBloqueiosSemanais`, não só no botão. `carregarConfiguracoes()` não mudou — já chamava essa função com `false` no boot da tela, então o comportamento "sempre começa escondido" veio de graça. Testes atualizados pra cobrir lista + botão juntos (179 no total, sem net-new) |
| 72 | Solicitação: o modal de confirmar cancelamento (bug #57) calculava sozinho se o Externo perdia o adiantamento (regra de 48h) e só avisava o resultado num texto — pedido: recepção escolhe manualmente o desfecho do depósito a cada cancelamento, sem cálculo/sugestão automática | `js/core/init.js`, `js/features/reservas/service.js`, `js/features/reservas/modal.js` | Novo `modalCancelarReserva(onEscolher)` em `init.js` (exposto via `window.modalCancelarReserva`, mesma infra de overlay/Escape/clique-fora de `modalConfirmar()`, mas com botões próprios — por isso não reaproveita a função genérica): título fixo "CANCELAR RESERVA" + 3 botões (SEM ESTORNO → `true`, ESTORNADO → `false`, SEM DEPÓSITO → `null`) + "CANCELAR" que só fecha sem cancelar a reserva. `cancelarReserva(id, depositoRetido)` ganha esse segundo parâmetro (default `null`) em vez de calculá-lo internamente; `_calcularDepositoRetido()`/`PRAZO_CANCELAMENTO_HORAS` removidos (dead code — nenhum outro fluxo usava). `deposito_retido` no banco continua com a mesma semântica (`true`/`false`/`null`), então `vw_reservas_detalhado`/Power BI não precisam de nenhuma mudança. 6 testes de `_calcularDepositoRetido` removidos (176 no total, sem perda de cobertura relevante — a função não existe mais) |
| 73 | Solicitação: campo de busca por nome no Log de Alterações; ícones dos itens (criado/editado/excluído/bloqueado) deveriam ser clicáveis pra ver antes/depois. Refinamento no mesmo dia: só EDITAR deve expandir — CRIAR não tem "antes" (nada existia), os demais não precisam do detalhe | `index.html`, `js/features/reservas/log.js` | Novo campo "Nome" na tela de Log — busca em TODAS as datas (não só a filtrada) via `.or()` do PostgREST em `dados_antes`/`dados_depois` (uma linha EXCLUIR só tem `dados_antes`), debounce 400ms; timeline agrupa por dia em vez de só hora quando os resultados cruzam datas. `.tl-icon` ganha `onclick`/classe `tl-clicavel` igual ao `.tl-card`, mas restrito a `acao === 'EDITAR'` com `_camposAlterados()` não-vazio (mesma regra de sempre — só chegou a existir um modo "retrato" pra CRIAR/EXCLUIR/DESBLOQUEAR num commit intermediário, revertido no mesmo dia por não fazer sentido pro usuário) |
| 74 | Reportado em teste manual: editar só o WhatsApp/Avulsa/Menu Degustação, ou editar o número de pessoas, não abria a opção de ver detalhes no Log — como se nada tivesse mudado. Duas causas raiz distintas, achadas na mesma investigação | `js/features/reservas/log.js`, `js/features/reservas/service.js` | **(1)** `_resumir()`/`LABELS_CAMPOS` (log.js) não rastreavam `whatsapp`/`avulsa`/`menuDegustacao` — editar só um desses nunca aparecia no diff porque o "antes" desses campos nunca era salvo no log. **(2)** Bug mais sério em `salvarReserva()` (service.js): `registrarLog('EDITAR', { id: dados.id, ...dados }, ...)` usava `dados` (o que acabou de ser SUBMETIDO pelo formulário) como "antes" — quando o certo era `dadosAntesReserva`, o estado real anterior já buscado no banco 2 linhas acima (`db.getReservaPorId()`), mas nunca repassado ao log. Resultado: toda edição comparava os dados novos contra eles mesmos (só com formatação diferente — string vs número), então nenhuma mudança de campo numérico/direto era detectada, mesmo tendo mudado de verdade (ex: paxs 2→4). Corrigido usando `dadosAntesReserva` como "antes". Só vale pra logs novos — edições antigas que só mudaram esses campos não têm o "antes" pra recuperar retroativamente. 2 testes novos (178 no total) |

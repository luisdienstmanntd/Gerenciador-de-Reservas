...
| `js/core/init.js` | **v2.2** | 2026-07-14 — branch "Alterar horário" do `btnSalvar` pergunta antes de criar linha nova quando o destino está sem linha base livre (bug #65). 2026-07-10 — chama `iniciarEscutaConfigSistema()` no boot (bug #56) |
...
| `js/features/reservas/modal.js` | **v3.16** | 2026-07-14 — `btnConfirmarData` trata o erro `semDisponibilidade` de `alterarData()` (bug #66). `recarregarReservas()` após `alterarData()` (bug #64). 2026-07-13 — menus de resumo e o seletor de ação (`abrirNova`) redesenhados com ícones SVG lineares + classes `.resumo-btn` (bugs #60, #61) |

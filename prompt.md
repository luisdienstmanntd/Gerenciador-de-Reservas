...
| 52 | Solicitação: padrão é atender 2 pessoas por linha/horário sem atrasar a cozinha. Reserva com 4+ adultos consome a capacidade de duas linhas, mas nada avisava/reservava esse espaço na próxima linha | `js/features/reservas/service.js`, `js/core/database.js`, `supabase/migrations/20260710140000_bloqueio_automatico.sql` | Nova coluna `reservas.bloqueio_origem_id` (FK pra si mesma, `on delete cascade`) identifica qual reserva gerou um bloqueio automático. `salvarReserva()`/`salvarApenasHorario()`/`alterarData()` chamam `_reconciliarBloqueioAutomatico()`: reserva com paxs>=4 bloqueia a próxima linha livre do bloco automaticamente; se a reserva for editada pra <4 pessoas ou mudar de bloco, o bloqueio criado é desfeito sozinho (exclusão da reserva de origem já limpa via `on delete cascade`, sem código extra). Se a próxima linha já estiver ocupada por outra reserva, ou não existir no bloco, só avisa a recepção — nunca força/remove espaço de terceiros |

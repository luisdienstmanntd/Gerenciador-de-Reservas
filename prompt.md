...
| `js/features/reservas/service.js` | **v4.4** | 2026-07-14 — `alterarData()` ganha a mesma checagem de capacidade/aviso de `salvarApenasHorario()` (bug #66). `salvarApenasHorario()` busca posição livre a partir de 0 no bloco de destino, não mais a partir da posição antiga; lança `semDisponibilidade` quando as linhas base do destino estão todas ocupadas (bug #65). 2026-07-10 — bloqueio automático de reserva grande (bugs #52, aplica em várias linhas + reavaliação após desbloqueio manual), respeitando o switch de Configurações (bug #56) |

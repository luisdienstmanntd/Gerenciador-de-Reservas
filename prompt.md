...
| `js/features/reservas/modal.js` | **v3.15** | 2026-07-14 — `recarregarReservas()` após `alterarData()` (bug #64). 2026-07-13 — menus de resumo e o seletor de ação (`abrirNova`) redesenhados com ícones SVG lineares + classes `.resumo-btn` (bugs #60, #61) |
| `js/features/reservas/service.js` | **v4.3** | 2026-07-14 — `salvarApenasHorario()` busca posição livre a partir de 0 no bloco de destino, não mais a partir da posição antiga; lança `semDisponibilidade` quando as linhas base do destino estão todas ocupadas (bug #65). 2026-07-10 — bloqueio automático de reserva grande (bugs #52, aplica em várias linhas + reavaliação após desbloqueio manual), respeitando o switch de Configurações (bug #56) |

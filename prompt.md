...
| `index.html` | — | Scripts Firebase (CDN) sem `integrity` (SRI, bug #36); login gate usa `onAuthStateChanged()` como fonte da verdade em vez de `localStorage`, chamando `recarregarReservas()` + `carregarHome()` ao confirmar usuário real (bugs #37, #38) |
| `css/style.css` | — | — |
| `firebase.json` / `.firebaserc` | — | Adicionados em 2026-07-03 — deploy de Hosting passa a ser reproduzível a partir deste repositório (`firebase deploy --only hosting`), projeto `osteria-di-lucca-links` (distinto do projeto `osteriadilucca-afea6` usado por Firestore/Auth em `database.js` — ver dívida técnica #3) |
